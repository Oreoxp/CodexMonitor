...
export function SettingsServerSection({
  ...
  return (
    ...
    >
        ...
        {isMobileSimplified && (
          <>
            <div className="settings-field">
              ...
              <div className="settings-mobile-remotes" role="list" aria-label={t("settings.server.savedRemotes")}>
                    ...
                    >
                      <div className="settings-mobile-remote-main">
                        <div className="settings-mobile-remote-name-row">
                          ...
                          {isActive && <span className="settings-mobile-remote-badge">{t("settings.server.active")}</span>}
                        </div>
                      ...
                      <div className="settings-mobile-remote-actions">
                        ...
                        <button
                          ...
                          disabled={index === 0}
                          aria-label={t("settings.server.moveUp", { name: entry.name })}
                        >
                          ↑
                        </button>
                        <button
                          ...
                          aria-label={t("settings.server.moveDown", { name: entry.name })}
                        >
                          ↓
                        </button>
                        <button
                          ...
                          aria-label={t("settings.server.deleteRemote", { name: entry.name })}
                        >
        ...
        <div className="settings-field">
          ...
          <div className="settings-field-row">
            ...
            <input
              ...
              placeholder={t("settings.server.tokenRequired")}
          ...
          <div className="settings-help">
            {isMobileSimplified
              ? t("settings.server.remoteHostMobileHelp")
              : t("settings.server.remoteHostDesktopHelp")}
          </div>
        </div>
        ...
        {isMobileSimplified && (
          <div className="settings-field">
            ...
            <div className="settings-field-row">
              ...
              >
                {mobileConnectBusy ? t("settings.server.connecting") : t("settings.server.connectAndTest")}
              </button>
            </div>
            {mobileConnectStatusText && (
        ...
        {!isMobileSimplified && (
          <div className="settings-field">
            ...
            <div className="settings-field-row">
              ...
              >
                {tcpDaemonBusyAction === "start" ? t("settings.server.starting") : t("settings.server.startDaemon")}
              </button>
              <button
                type="button"
                className="button settings-button-compact"
                ...
                disabled={tcpDaemonBusyAction !== null}
              >
                {tcpDaemonBusyAction === "stop" ? t("settings.server.stopping") : t("settings.server.stopDaemon")}
              </button>
              <button
                type="button"
                className="button settings-button-compact"
                ...
                disabled={tcpDaemonBusyAction !== null}
              >
                {tcpDaemonBusyAction === "status" ? t("settings.server.refreshing") : t("settings.server.refreshStatus")}
              </button>
            </div>
            {tcpRunnerStatusText && <div className="settings-help">{tcpRunnerStatusText}</div>}
            {tcpDaemonStatus?.startedAtMs && (
              <div className="settings-help">
                {t("settings.server.startedAt")}: {new Date(tcpDaemonStatus.startedAtMs).toLocaleString()}
              </div>
            )}
            <div className="settings-help">
              {t("settings.server.startDaemonHelp")} <code>0.0.0.0:&lt;port&gt;</code>, {t("settings.server.startDaemonHelpSuffix")}
            </div>
          </div>
        )}
        ...
        {!isMobileSimplified && (
          <div className="settings-field">
            ...
            <div className="settings-field-row">
              ...
              >
                {tailscaleStatusBusy ? t("settings.server.checking") : t("settings.server.detectTailscale")}
              </button>
              <button
                type="button"
                className="button settings-button-compact"
              ...
              >
                {tailscaleCommandBusy ? t("settings.server.refreshing") : t("settings.server.refreshDaemonCommand")}
              </button>
              <button
                type="button"
                className="button settings-button-compact"
              ...
              <>
                ...
                <div className="settings-help">
                  {tailscaleStatus.installed
                    ? `${t("settings.server.version")}: ${tailscaleStatus.version ?? t("settings.server.unknown")}`
                    : t("settings.server.installTailscale")}
                </div>
                {tailscaleStatus.suggestedRemoteHost && (
                  <div className="settings-help">
                    {t("settings.server.suggestedRemoteHost")}: <code>{tailscaleStatus.suggestedRemoteHost}</code>
                  </div>
                )}
            ...
            {tailscaleCommandPreview && (
              <>
                <div className="settings-help">
                  {t("settings.server.commandTemplate")}
                </div>
      ...
      <div className="settings-help">
        {isMobileSimplified
          ? t("settings.server.infrastructureHelpMobile")
          : t("settings.server.infrastructureHelpDesktop")}
      </div>
      {addRemoteOpen && (
        <ModalShell
        ...
        >
          ...
          <div className="settings-add-remote-actions">
            ...
            <button
              ...
              disabled={addRemoteBusy}
            >
              {addRemoteBusy ? t("settings.server.connecting") : t("settings.server.connectAndAdd")}
            </button>
          </div>
        </ModalShell>
      )}
      {pendingDeleteRemote && (
        ...
        >
          <div className="settings-delete-remote-title">{t("settings.server.deleteRemoteTitle")}</div>
          <div className="settings-delete-remote-message">
            {t("settings.server.deleteRemoteMessage", { name: pendingDeleteRemote.name })}
          </div>
